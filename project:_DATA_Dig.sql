...
SELECT name, SUM(intelligence * fighting_Skills) as Strong_Point 
FROM marvels
GROUP BY name
HAVING Strong_Point > 20;

SELECT COUNT(*) as 'Gender of Male' FROM marvels WHERE gender LIKE "%male%";

SELECT name,hometown,alignment FROM marvels WHERE hometown LIKE "%USA%" AND alignment LIKE "%Good%";

SELECT ROUND(AVG(height_m),2) as 'AVERAGE HEIGHT_M' FROM marvels;

SELECT name, MIN(popularity) as 'Least Popularity' FROM marvels;

SELECT name, MAX(weight_kg) as 'Most weight'FROM marvels;

SELECT height_m FROM marvels WHERE name = 'Rogue'; 

SELECT name, height_m FROM marvels WHERE height_m NOT IN (SELECT height_m FROM marvels WHERE height_m <= '1.98');

SELECT COUNT(*),
    CASE
        WHEN fighting_Skills > 4 THEN "Big Than Iron Man"
        WHEN fighting_Skills = 4 THEN "Same"
        WHEN fighting_Skills < 4 THEN "Smaller Than Iron Man"
    END AS "relative_skill_to_Iron_Man"
FROM marvels
GROUP BY relative_skill_to_Iron_Man;
